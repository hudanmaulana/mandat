(function (global, factory) {
	if (typeof define === "function" && define.amd) {
		define('/App/Taskboardlist', ['exports', 'Site'], factory);
		...
		babelHelpers.createClass(Stage, [{
			...
			value: function getTpl(title,id) {
				...
				return '\n' +
					...
					'</header>\n' +
					'<header class="taskboard-stage-header-two">' +
					'\t<form action="'+base_url+'rn/taskboard/detailtask" method="post" target="_blank">\n' +
					'<input type="hidden" id="id_divisi" value="' + id + '"name="divisi">\n' +
					'<button type="submit" class="btn btn-default btn-xs btn-md empty-btn waves-effect waves-classic">View List</button>\n' +
					'</form>' +
					'</header> '+
			...
			value: function bindRenameSaveBtn() {
				this.$renameSaveBtn.on('click', function () {
					...
					$.ajax({
						type:"POST",
						url  :base_url+"rn/taskboard/RenameSaveGroup",
						dataType : "JSON",
					...
					bootbox.confirm({
						...
						callback: function (result) {
							if(result){
								$.ajax({
									type:"POST",
									url  :base_url+"rn/taskboard/BindDeleteGroup",
									dataType : "JSON",
			...
			value: function bindAddItemBtn() {
				...
				this.$addItemBtn.on('click', function () {
					...
					if ($input.val().length !== 0) {
						...
						$.ajax({
							type:"POST",
							url  :base_url+"rn/taskboard/AddNewTaskTodo",
							dataType : "JSON",
	...
	var sildePaneldefaultsOptions = {
		...
		afterLoad: function afterLoad(object) {
			...
			this.$panel.find('#task-description');
	...
	var slidePanel = {
		...
		handleSelective: function handleSelective($target, selected) {
			...
			$.getJSON(base_url+'rn/taskboard/get_memberv2/'+divisiid, function (data) {
				...
				$target.selective({
					...
					onAfterItemAdd: function onAfterItemAdd() {
						...
						$.ajax({
							type:"POST",
							url  :base_url+"rn/taskboard/AddMember",
							dataType : "JSON",
					...
					onAfterItemRemove: function onAfterItemRemove() {
						...
						$(document).on('click', '.addMember-remove', function () {
							...
							$.ajax({
								type:"POST",
								url  :base_url+"rn/taskboard/DeleteMember",
								dataType : "JSON",
		...
		handlePriority: function handlePriority() {
			...
			$(document).on('click', '[name="priorities"]', function () {
				...
				$.ajax({
					type:"POST",
					url  :base_url+"rn/taskboard/ChangePriorityTask",
					dataType : "JSON",
		...
		handleEditor: function handleEditor() {
			...
			$(document).on('click', '.task-main-editor-save', function () {
				...
				$.ajax({
					type:"POST",
					url  :base_url+"rn/taskboard/UpdateDescriptionTask",
					dataType : "JSON",
		...
		handleSubtasks: function handleSubtasks() {
			...
			$(document).on('click', '.subtask-add-save', function () {
				...
				if (value.length === 0) {
					...
					$.ajax({
						type:"POST",
						url  :base_url+"rn/taskboard/AddSubTask",
						dataType : "JSON",
				...
				bootbox.confirm({
					...
					callback: function (result) {
						if(result){
							$.ajax({
								type:"POST",
								url  :base_url+"rn/taskboard/BindDeleteSubTask",
								dataType : "JSON",
		...
		handleDatepicker: function handleDatepicker() {
			$(document).on('click', '.due-date-save', function () {
				...
				if (value.length > 0) {
					...
					$.ajax({
						type:"POST",
						url  :base_url+"rn/taskboard/UpdateDuedateTask",
						dataType : "JSON",
		...
		handleAttachment: function handleAttachment() {
			...
			$(document).on('click', '#fileuploadToggle', function () {
				...
				$("#fileupload").change(function(){
					...
					$.ajax({
						url  :base_url+"rn/taskboard/AddAttachment",
						type: "POST",
						data: formData,
						processData: false,
						contentType: false,
				...
				bootbox.confirm({
					...
					callback: function (result) {
						if(result){
							$.ajax({
								type:"POST",
								url  :base_url+"rn/taskboard/BindDeleteAttachment",
								dataType : "JSON",
	...
	var AppTaskboard = function (_Site) {
		...
		babelHelpers.createClass(AppTaskboard, [{
			...
			value: function createStageList() {
				...
				$.getJSON(base_url+'rn/taskboard/apiv2/'+divisiid, function (data) {
			...
			value: function bindStageCreateBtn() {
				...
				this.$stageCreateBtn.on('click', function () {
					...
					$.ajax({
						type:"POST",
						url  :base_url+'rn/taskboard/AddGroupTodov2/'+divisiid,
						dataType : "JSON",
