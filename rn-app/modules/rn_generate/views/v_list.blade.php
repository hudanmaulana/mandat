    ...
    <div id="page_content" ng-controller="listController">
        ...
        <div id="page_content_inner" style="display: none;">
                        ...
                        <div class="md-card-content">
                            <div class="uk-grid" data-uk-grid-margin="">
                                ...
                                <div id="options-content" class="uk-width-medium-1-2">
                                    <a href="<?php echo '{{ base_url("cms/$module/add") }}'?>" title="Add Post" class="md-btn md-btn-success"><i class="material-icons">add</i>Add </a>
                    ...
                    <div class="uk-modal-header">
                        <div class="uk-clearfix">
                            <div class="uk-float-left">
                                <h3 class="uk-modal-title">Search <?php echo '{{ $subject }}'?></h3>
                            </div>
                            ...
                            <div class="uk-form-row">
                                <div class="md-input-wrapper">
                                    <select id="search-column" class="md-input">
                                        @foreach ($fields as $f)
                                        @if($f->primary_key!=1) <?php echo PHP_EOL;?> <option value="{{ $f->name }}">{{ $f->alias }}</option> @endif
                                        @endforeach
                                    </select>
                                </div>
                            </div>
            ...
            <div class="md-card uk-margin-medium-bottom">
                <div class="md-card-toolbar"> <h3 class="md-card-toolbar-heading-text"> List data  <?php echo '{{ $subject }}'?></h3> </div>
                <div class="md-card-content">
                    <div class="uk-overflow-container">
                        <table class="uk-table uk-table-nowrap table_check" style="margin-bottom: 68px;">
                            ...
                            <tr>
                                ...
                                @foreach ($fields as $f)
                                @if($f->primary_key!=1)
                                        <?php echo PHP_EOL;?> <th>{{ $f->alias }}</th>
                                @endif
                                @endforeach
                                <th>Actions</th>
                            </tr>
                            ...
                            <tr ng-repeat="t in data">
                                @foreach ($fields as $f)
                                    @if($f->primary_key==1)
                                        <?php echo PHP_EOL;?> <td><input class="check_row" type="checkbox" name="checkbox_{{ "t.$f->name" }}" ng-checked="checkbox" ng-click="updateSelection($event, t.{{ $f->name }})" title="Select"></td>
                                    @endif
                                @endforeach
                                ...
                                @if($field->type=='tinyint')
                                    <?php echo PHP_EOL;?> <td ng-if="t.{{ $field->name }}==1"><span class="uk-badge uk-badge-success">Yes</span></td>
                                    <?php echo PHP_EOL;?> <td ng-if="t.{{ $field->name }}!=1"><span class="uk-badge uk-badge-danger">No</span></td>
                                @else
                                    <?php echo PHP_EOL;?> <td>{{ "@{{t.$field->name" }}}}</td>
                                @endif
                                @endif
                                @endforeach
                                        ...
                                        <td>
                                            <div class="uk-button-dropdown" data-uk-dropdown="{mode:'click'}" aria-haspopup="true" aria-expanded="false">
                                                ...
                                                <div class="uk-dropdown uk-dropdown-small uk-dropdown-bottom" style="">
                                                    <ul class="uk-nav uk-nav-dropdown">
                                                        <li class="edit-row">
                                                            <a href="<?php echo '{{ base_url("cms/$module/edit") }}'?>/{{ "@{{t.$field->name" }}}}" title="Edit"><i class="material-icons">mode_edit</i> Edit</a>
                                                        </li>
                    ...
                    <div class="uk-grid" data-uk-grid-margin="" style="margin-top: 30px;">
                        ...
                        <div class="uk-width-large-1-4 uk-width-medium-1-2">
                            <div style="margin-top:15px;" class="uk-input-group"> Page <span id="page-starts-from"><?php echo '@{{pageStart}}'?></span> from <span id="total_items"><?php echo '@{{maxPage}}'?></span> Total Pages</div>
                        </div>
                        <div class="uk-width-large-1-4 uk-width-medium-1-2">
                            <div class="uk-input-group">
                                <div class="md-input-wrapper md-input-filled">
                                    <input class="md-input" name="tb_crud_page" type="text" value="<?php echo '@{{pageStart}}'?>" id="tb_crud_page" disabled>
                                    <span class="md-input-bar"></span>
                                </div>
                            </div>
                        </div>
                        <div class="uk-width-large-1-4 uk-width-medium-1-2">
                            <div style="margin-top:15px;" class="uk-input-group"> Total Data <?php echo '@{{ totalRows | number }}'?></div>
                        </div>
                    </div>
